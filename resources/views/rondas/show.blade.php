        ...
        <div class="row">
            ...
            @if ($ronda->abierta)
                <div class="col-12 col-md-4" id="s_novedad">
                    ...
                    <input type="file" capture="environment" id="input_image" accept="image/png, image/jpeg"
                        class="form-control mb-2">
                    <div class="d-grid gap-2">
                        <button type="button" class="btn btn-lg btn-success" onClick="aceptar_formulario()"
                            id="btn-aceptar">Aceptar</button>
                        <button type="button" class="btn btn-lg btn-warning" onClick="vaciar_novedad()"
                            id="btn-vaciar">Borrar Campo</button>
                    </div>
                </div>
            @endif
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (fileInput) {
            fileInput.addEventListener('change', e => {
                ...
                btn_aceptar.innerHTML =
                    '<div class="spinner-border spinner-border-sm" role="status"><span class="visually-hidden"></span></div> Procesando...';
        ...
        @foreach ($ronda->checkpoints as $geo)
            punto_visitado = L.marker([{{ $geo->latitud }}, {{ $geo->longitud }}], {
                icon: @if (count($geo->images) > 0)
                    icono_imagenes
                @elseif ($geo->novedad) icono_novedades
                @else
                    icono_sin_novedades
                @endif
            }).addTo(myMap);
        ...
        function aceptar_formulario() {
            ...
            btn_vaciar.disabled = true;
            btn_aceptar.innerHTML =
                `<div class="spinner-border spinner-border-sm" role="status"><span class="visually-hidden"></span></div> Subiendo...`;
            ...
            return new Promise((resolve, reject) => {
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(function(position) {
                            ...
                            document.getElementById('btn_obtener_ubicacion').innerHTML =
                                `Obtener ubicación <i class="bi bi-geo-alt"></i>`;
                            myMap.panTo(new L.LatLng(position.coords.latitude, position.coords
                                .longitude));
